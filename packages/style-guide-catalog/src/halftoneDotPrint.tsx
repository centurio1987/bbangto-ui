...
export const halftoneDotPrintStyleGuide: StyleGuide = {
  ...
  visualMotif,
  meta: {
    displayName: 'Halftone_Dot_Print_01',
    trendIndex: 36,
    family: 'nostalgia',
    priority: 'P2',
    summary: '밝은 페이퍼 + CMYK 4원색 망점 스크린톤, 점의 크기·밀도·스크린 각으로 톤을 만드는 인쇄 분해 그래픽.',
    tags: ['light', 'vivid', 'textured', 'retro', 'sharp', 'geometric', 'typographic'],
    mood: { formality: 2, energy: 4, warmth: 3, density: 4, ornament: 4 },
    characteristics: {
      cornerRadius: 'sharp',
      borderWeight: 'thin',
      shadow: 'none',
    ...
    domains: ['editorial', 'portfolio', 'creative-agency', 'marketing', 'landing'],
    useWhen: [
      'CMYK 망점·색분해 인쇄 미학으로 포스터·에디토리얼 표지를 만들 때 쓴다.',
      '갤러리·포트폴리오 이미지 그리드에 인쇄 질감과 개성을 입힐 때 쓴다.',
      '굵은 그로테스크 헤드라인과 레트로 프린트 무드가 필요한 랜딩에 쓴다.',
      '도트 밀도 그라데이션으로 그림자 없이 위계·입체감을 표현하고 싶을 때 쓴다.',
    ],
    avoidWhen: [
      '데이터 밀도 높은 대시보드·관리 UI처럼 조용한 가독성이 우선일 때 피한다.',
      '작은 텍스트·핵심 UI 면적이 넓어 도트 스크린이 가독성을 해칠 때 피한다.',
      '기업·핀테크처럼 절제된 신뢰감이 핵심일 때 피한다.',
    ],
    accessibility: {
      contrastIntent: 'aa',
    ...
    related: ['risograph-print-01', 'halftone-glitch-colorsep-01'],
  },
};
