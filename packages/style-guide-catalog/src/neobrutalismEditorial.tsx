 ...
 * IBM Plex Sans/Mono. 다른 preset과 동일한 체계를 따른다 — 파일명 <슬러그 camelCase>.tsx,
 * export 트리오 <슬러그 camelCase>StyleGuide/<Prefix>Showcase/<슬러그 camelCase>Wrappers.
...
const inkOffset = (n: number) => `${n}px ${n}px 0 ${NEO.ink}`;
/** 골드 오프셋 그림자 — 버튼 모티프의 시그니처. */
const NEO_OFFSET_SHADOW = `3px 3px 0 ${NEO.gold}`;
...
const foundations = makeFoundations({
  ...
  neutral: {
    0: NEO.ink, 10: '#221F1A', 20: '#3A3833', 30: '#4A463C', 40: NEO.subtle,
    50: '#857F70', 60: NEO.faint, 70: '#B9B19C', 80: '#D8CFB5', 90: NEO.creamSunken,
    95: NEO.cream, 99: NEO.paper, 100: '#ffffff',
  },
  semantic: makeSemantic({
    ...
    error: { base: '#C0392B', hover: '#A93226', active: '#922B21', subtle: '#F5D8D3', foreground: NEO.cream },
    success: { base: '#5C7A4A', hover: '#4E6840', active: '#3F5534', subtle: NEO.green, foreground: NEO.cream },
    warning: { base: '#B8860B', hover: '#A2770A', active: '#8A6508', subtle: NEO.gold, foreground: NEO.ink },
    disabled: { background: NEO.creamSunken, foreground: NEO.faint, border: '#C9BFA3' },
    category: {
      planning: '#5C7A4A', architecture: NEO.ink, strategy: '#C0392B', tech: '#B8860B',
      design: NEO.pink, research: NEO.green, quality: '#5C7A4A', leadership: NEO.gold,
    },
  }),
});

const extendedFoundations: Record<string, string> = {
  '--bbangto-ext-ink': NEO.ink,
  '--bbangto-ext-paper': NEO.paper,
  '--bbangto-ext-accent': NEO.gold,
  '--bbangto-ext-accent-app': NEO.green,
  '--bbangto-ext-accent-infra': NEO.pink,
  '--bbangto-ext-border-width': NEO.borderWidth,
  '--bbangto-ext-offset-shadow': NEO_OFFSET_SHADOW,
};
...
const midnightFoundations = makeColorway(foundations, {
  ...
  semantic: makeSemantic({
    bg: NEO.ink, bgElevated: '#2A2823', bgSunken: '#141310', overlay: 'rgba(0,0,0,0.6)',
    fg: NEO.cream, fgMuted: '#C9C2AE', fgSubtle: '#9A9483', fgInverse: NEO.ink,
    border: NEO.cream, borderMuted: '#3A3833', borderStrong: NEO.cream, focus: NEO.gold,
    primaryBase: NEO.gold, primaryHover: NEO.goldHover, primaryActive: '#C9A63F',
    primarySubtle: '#3A3833', primaryFg: NEO.ink,
    accent: NEO.gold, accent2: NEO.green, accent3: NEO.pink,
    warning: { base: NEO.gold, hover: NEO.goldHover, active: '#C9A63F', subtle: '#3A3833', foreground: NEO.ink },
  }),
});
const midnightExt: Record<string, string> = {
  '--bbangto-ext-ink': NEO.cream,
  '--bbangto-ext-paper': '#2A2823',
  '--bbangto-ext-accent': NEO.gold,
  '--bbangto-ext-accent-app': NEO.green,
  '--bbangto-ext-accent-infra': NEO.pink,
  '--bbangto-ext-border-width': NEO.borderWidth,
  '--bbangto-ext-offset-shadow': `3px 3px 0 ${NEO.cream}`,
};
...
const berryFoundations = makeColorway(foundations, {
  ...
  semantic: makeSemantic({
    bg: NEO.cream, bgElevated: NEO.paper, bgSunken: NEO.creamSunken, overlay: 'rgba(28,27,23,0.55)',
    fg: NEO.ink, fgMuted: NEO.muted, fgSubtle: NEO.subtle, fgInverse: NEO.cream,
    border: NEO.ink, borderMuted: '#3A3833', borderStrong: NEO.ink, focus: BERRY,
    primaryBase: NEO.ink, primaryHover: NEO.inkHover, primaryActive: NEO.inkActive,
    primarySubtle: '#E6B3C3', primaryFg: NEO.cream,
    accent: BERRY, accent2: NEO.green, accent3: NEO.pink,
  }),
});
...
const STYLE_ID = 'bbangto-neobrutalism-editorial-01-motif';
const CSS = `
.bbangto-neo-btn {
...
@media (prefers-reduced-motion: reduce) {
  .bbangto-neo-btn { transition: none !important; }
  .bbangto-neo-btn:hover,
  .bbangto-neo-btn:active { transform: none; }
}
`;
...
const wrapperComponents = makeMotifWrappers({
  styleId: STYLE_ID,
  css: CSS,
  buttonClass: 'bbangto-neo-btn',
  cardClass: 'bbangto-neo-card',
  displayPrefix: 'Neobrutalism',
  tag: {
    defaultTone: 'accent',
    baseStyle: {
...
export const neobrutalismEditorialStyleGuide: StyleGuide = {
  name: 'neobrutalism-editorial-01',
