...
export const warpedCheckerboardStyleGuide: StyleGuide = {
  ...
  meta: {
    ...
    priority: 'P3',
    summary: '직교 체크/깅엄 격자를 파도처럼 액화·왜곡한 풀블리드 디스토션 배경 + 솔리드 패널 분리, 딥 레드×베이비 핑크 2색의 표현적 룩북/랜딩 미학.',
    tags: ['vivid', 'pastel', 'rounded', 'organic', 'retro', 'playful', 'animated'],
    mood: { formality: 2, energy: 4, warmth: 4, density: 3, ornament: 4 },
    characteristics: {
      cornerRadius: 'round',
      borderWeight: 'bold',
      shadow: 'none',
    ...
    domains: ['landing', 'marketing', 'portfolio', 'ecommerce', 'creative-agency'],
    useWhen: [
      '이벤트·굿즈·패션 룩북처럼 강한 개성과 임팩트가 필요한 랜딩을 만들 때 쓴다.',
      '프로필·링크인바이오처럼 시각적 시그니처가 곧 브랜드인 페이지에 쓴다.',
      '히어로 배경을 유체 왜곡 패턴으로 채우고 콘텐츠는 솔리드 패널로 분리할 때 쓴다.',
      'Y2K·레트로 감성의 말랑한 라운드 그로테스크 무드를 원할 때 쓴다.',
    ],
    avoidWhen: [
      '표·폼처럼 정보 밀도 높고 가독성이 최우선인 화면에는 피한다.',
      'B2B SaaS·핀테크처럼 신뢰·절제된 톤이 필요할 때 피한다.',
      '적록색맹·저시력 대비가 엄격히 요구되는 접근성 우선 UI에는 피한다.',
    ],
    accessibility: {
      contrastIntent: 'aa',
    ...
    related: ['op-art-kinetic-01', 'memphis-postmodern-01', 'y2k-futurism-01'],
  },
};
