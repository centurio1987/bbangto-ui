...
export const neumorphismSoftStyleGuide: StyleGuide = {
  ...
  visualMotif,
  meta: {
    displayName: 'Neumorphism_Soft_01',
    trendIndex: 2,
    family: 'depth-material',
    priority: 'P1',
    summary: '배경과 같은 단색 표면을 이중 그림자로 볼록/오목 압출한 부드러운 촉각 UI.',
    tags: ['light', 'muted', 'monochrome', 'rounded', 'depth', 'minimal', 'mono'],
    mood: { formality: 3, energy: 1, warmth: 2, density: 2, ornament: 2 },
    characteristics: {
      cornerRadius: 'round',
      borderWeight: 'none',
      shadow: 'soft',
    ...
    domains: ['saas', 'dashboard', 'entertainment', 'portfolio', 'fintech'],
    useWhen: [
      '물리 버튼 같은 눌림 피드백의 부드러운 촉각 인터페이스를 원할 때 쓴다.',
      '설정 패널·미디어 플레이어·계산기형 키패드처럼 컨트롤이 적은 화면에 쓴다.',
      '단일 색조로 시각 소음을 줄인 차분한 인상을 만들고 싶을 때 쓴다.',
    ],
    avoidWhen: [
      '텍스트·데이터 밀도가 높아 강한 대비와 가독성이 우선일 때 피한다.',
      'WCAG 대비를 엄격히 지켜야 하는 접근성 민감 서비스에 피한다.',
      '명확한 위계·강조가 필요한 정보 중심 화면에 피한다.',
    ],
    accessibility: {
      contrastIntent: 'low',
    ...
    related: ['claymorphism-playful-01', 'skeuomorphism-tactile-01', 'glassmorphism-aurora-01'],
  },
};
