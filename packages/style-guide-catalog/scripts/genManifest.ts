 ...
 *
 * styleGuideCatalog(SSOT)에서 매니페스트를 파생해 패키지 루트에 결정적 JSON으로 쓴다.
 * 카탈로그 객체는 구조만 읽으므로(렌더 없음) Node에서 안전하게 실행된다.
 * 단, wrapper/showcase가 core 런타임을 import하므로 core dist가 빌드돼 있어야 한다.
 */
import { writeFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';
import { styleGuideCatalog, buildManifest, serializeManifest } from '../src/index';
