/**
 * Visualization catalog manifest 생성기 — viz style guide 배열을 AI가 파일 하나로 읽는
 * 압축 매니페스트로 투영한다. UI(style-guide-catalog)의 동형 생성기를 국소 복제한 것으로,
 * 로직은 동일하며 `VisualizationStyleGuide`도 `CatalogEntryLike`를 구조적으로 만족한다.
 *
