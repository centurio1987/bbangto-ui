 ...
 *
 * NOTE: UI style-guide-catalog의 select.ts를 국소 복제한 것(generic이라 로직 동일). viz→ui-catalog
 * 런타임 결합을 피하려 복제하며(KAN-021 manifest.ts 선례), 복제 drift는 select.test.ts의 parity
 * 테스트(UI helper를 devDep로 import해 동일 fixture deep-equal)가 CI에서 가드한다.
 */
import type {
  StyleGuideMeta,
  StyleFamily,
