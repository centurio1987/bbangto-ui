...
> **ORD-011 구현 완료(P2 22건)**: E 차트 `StackedBarChart`·`AreaChart`·`ScatterPlot`·`Histogram`·`DotPlot`·`WaterfallChart`·`Heatmap`·`ChoroplethMap`(caller path) + A 다이어그램 `UseCaseDiagram`(신규 `ActorGlyph`)·`C4DynamicDiagram`·`C4SystemLandscapeDiagram`·`DataFlowDiagram`·`ActivityDiagram`(🔶→✅) + `ConceptMap`(🔶→✅)·`Fishbone` + 패턴 `Funnel`·`ListInfographic`·`AnnotatedIllustration`·`SwotMatrix`·`OnionDiagram` + 모드 확장 `Cycle.flywheel`(VT-708 🔶→✅)·`Statistics.waffle`(VT-513 🔶→✅). 신규 순수 geometry `stack`/`histogram`/`waterfall`/`funnel`/`fishbone`(vitest 단위). 전부 §C-2 공통 계약 준수(신규 paint 채널 0). 커버리지 76/90(84%). 잔여 P3 백로그(신규 프리미티브 필요 위주).
> **ORD-012 구현 완료(P3 11건 — 인벤토리 백로그 소진)**: E 차트 `Boxplot`·`ChordDiagram` + A 다이어그램 `UMLPackageDiagram`(신규 `folder` shape)·`DMNDiagram`(knowledgeSource/bkm path 헬퍼)·`BPMNCollaborationDiagram`(🔶→✅·message flow dashed)·`ArchiMateViewpointDiagram`(motivation/strategy/implementation viewpoint) + C `WorkBreakdownStructure`(🔶→✅·WBS 십진 번호) + 패턴 `InformationalInfographic`·`Iceberg`·`BusinessModelCanvas`(표준 비대칭 9블록)·`Honeycomb`. 신규 순수 geometry `boxplot`(Tukey)/`chord`(반환 규약 선고정)/`iceberg`(폴리곤 사다리꼴 밴드)/`hexgrid`(벌집 패킹) + `tree.wbsNumbering`(다중 루트·순환 방어)(vitest 단위 +26). 전부 §C-2 공통 계약 준수(신규 paint 채널 0). 커버리지 **87/90(97%)** — 범위 외 ⛔ 3 제외 시 채택 유형 100%. **P1·P2·P3 잔여 0.**
...
> **KAN-015 구현 완료(G6 2종)**: `Kruchten4Plus1View`(영역별 `slots`=중첩 프리셋 우선, `data`=불릿 폴백, 영역 단위 no-merge; 3행 배치 = 상단 2코너·중앙 Scenarios 밴드·하단 2코너) + `ViewpointFrame`(헤더밴드=viewpoint·concerns[]·stakeholders[]·modelKinds[], body 슬롯=중첩 view, 미지정 시 플레이스홀더). 둘 다 §C-2 공통 계약 준수(신규 아톰/geometry/paint 채널 0 — 기존 `Canvas`/`Boundary`·`vvar`·`parseViewBox`만 재사용). 슬롯은 절대좌표 nested `<svg x/y/w/h>`로 다른 프리셋을 조합. **주의**: 42010 `ViewpointFrame`은 ArchiMate 고유 viewpoint(`ArchiMateViewpointDiagram`, VT-121)와 별개 개념 — description/주석에 명시. 4 게이트 green(typecheck·build·test 1106·storybook build).
