...
- `KAN-037` viz-print-ink 패밀리 — Halftone_Print + Glitch_Duotone viz 가이드 + riso 리네임 (KAN-034 P2) — 생성:ai · 최종:ai · 갱신:2026-07-24
  - 메모: KAN-034 결정: 인쇄 계열 family 그룹핑. 첫 단계=shipped viz-riso-print→viz-print-ink 리네임(STYLE_FAMILIES union + STYLE_FAMILY_LABELS + risoPrint.tsx meta.family + manifest/트렌드표 regen; manifest·displayName·accessibility 테스트 통과 확인). 이어 Halftone_Print(UI#36 CMYK 망점 SVG pattern) + Glitch_Duotone(UI#33 마젠타/시안 채널 오프셋 duotone) 2 가이드 저작, 둘 다 meta.family=viz-print-ink 공유. colorway 통합 불가(모티프=별도 wrapper). Tier A. 스펙: viz-style-expansion.md §7-3/§7-5.
- `KAN-038` viz-soft-puffy 패밀리 — Neumorphic + Clay_Playful + Kawaii_Pastel viz 가이드 (KAN-034 P2) — 생성:ai · 최종:ai · 갱신:2026-07-24
  - 메모: KAN-034 결정: 소프트 계열 family 그룹핑. 3 가이드 모두 meta.family=viz-soft-puffy 공유·별도 wrapper: Neumorphic(UI#2 동색 이중그림자 압출, 저대비 리스크 — ink↔bg/tag↔fill ≥4.5:1 유의), Clay_Playful(UI#3 파스텔 퍼피 inset filter), Kawaii_Pastel(UI#17 파스텔+마스코트 글리프=신규 데코 wrapper, Tier B). colorway 통합 불가(shadow/radius/wrapper 상이). Tier A/B. 스펙: §7-3/§7-5.
