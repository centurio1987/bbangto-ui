...
- `KAN-041` foundations 잔여 73종 FoundationMeta 전량 backfill → census infra-pilot→covered 승격(pending 0) — 생성:ai · 최종:ai · 갱신:2026-07-25
  - 메모: KAN-035 후속. FoundationMeta 스키마·게이트·셀렉터·서브패스는 동결(stable). 잔여 73 foundation에 meta 저작(Workflow 배치, KAN-021/040 선례). DoD: pending 0 + metadata-coverage.json foundation status infra-pilot→covered + selector 기대 fixture 보강 + FOUNDATION_METADATA_STRATEGY 롤아웃 표 갱신. KAN-035 인프라 위 additive(파일 미변경, registry에 meta 추가).
...
## 완료
- `KAN-020` visualization **유형(패턴/템플릿) 축** 매니페스트 + selector 인프라 구현 — 생성:유저 · 최종:유저 · 갱신:2026-07-25
