...
- **`@bbangto-ui/core`**: UI 컴포넌트 라이브러리 및 커스텀 생성 아이콘
- **`@bbangto-ui/tokens`**: 디자인 토큰 시스템 (Colors, Spacing, Typography 등)
...
이 패키지를 프로젝트에 설치하려면, 패키지 매니저(예: pnpm)를 통해 `@bbangto-ui/core`와 `@bbangto-ui/tokens`를 추가하세요. (추후 npm 배포 시 사용 가능)

```bash
pnpm install @bbangto-ui/core @bbangto-ui/tokens
```
...
```tsx
import { Button, HomeIcon, SettingsIcon } from '@bbangto-ui/core';
