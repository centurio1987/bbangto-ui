...
- [ ] `S1` RED 고정 — routing 단위 테스트 신설 + P1·P3 chromium 실측
...
- [ ] `S2` P1 수정 — `orthogonalPath` 종단 세그먼트 ε 규칙
...
- [ ] `S3` P1 회귀 스윕 — 자체 저장소 축 정렬 엣지 전수 + 클라이언트 게이트 대조
...
- [ ] `S4` P2 — `contentBox(shape, bbox)` 순수 함수 export
...
- [ ] `S5` P2 — `NodeLabel`이 콘텐츠 박스를 받게 한다
...
- [ ] `S6` P4 — `typography.labelFont` 토큰 + 스크립트 인식 해석 헬퍼
...
- [ ] `S8` P3 — Boundary 라벨 배치 (S1 실측 결과에 따라 형태 확정)
